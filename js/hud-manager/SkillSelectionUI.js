...
import { getSkillIcon, getSkillIconEmoji } from '../config/skill-icons.js';
...
export class SkillSelectionUI extends UIComponent {
    ...
    populatePrimaryAttacks() {
        ...
        const primaryAttacksHTML = this.orderedSkills.primaryAttacks.map(skill => {
            ...
            const icon = getSkillIconEmoji(skill.name, iconData);
    ...
    updatePreview() {
        ...
        const battleSkillsHTML = battleSkills.map(skill => {
            ...
            const icon = skill.isEmpty ? "+" : (skill.isMore ? "..." : getSkillIconEmoji(skill.name, iconData));
